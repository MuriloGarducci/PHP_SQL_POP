...
if (isset($_REQUEST["excluir"]) && $_REQUEST["excluir"] == true) {
    if (isset($_POST["bandas_id"]) && !empty($_POST["bandas_id"])) {

        $sql = "DELETE FROM bandas WHERE id = ?";
        $stmt = $connection->prepare($sql);
        $stmt->bindParam(1, $_POST["bandas_id"]);
        $stmt->execute();
...
<body>
    <div class="stars"></div>
    <h2>Exclusão de Bandas</h2>

    <?php if ($excluir == true){ ?>
        <p>Banda excluída com sucesso!</p>
        <p><a href='bandas.php'>Visualizar bandas restantes</a></p>
    <?php }else{ ?>
        <?php if (isset($erro)){?>
