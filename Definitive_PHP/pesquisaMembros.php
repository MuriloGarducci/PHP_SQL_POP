...
<body>
    ...
    <div class="form-container">
        <form action="" method="get">
            <label for="search">Pesquisar Integrante:</label>
        ...
        <?php try {
            ...
            if (isset($_GET['search']) && $_GET['search'] != '') {
                ...
                $rs = $connection->prepare("SELECT nome FROM integrantes WHERE LOWER(nome) LIKE :search");
                $rs->bindParam(':search', $search_term);
                $rs->execute();
                $integrantes = $rs->fetchAll(PDO::FETCH_OBJ);
                if (count($integrantes) == 0) {
                    echo '<p class="error">Não foi encontrado o integrante com o nome "' . $search_term . '"</p>';
                } else {
                    echo '<h2>Resultado da Pesquisa:</h2>';
                    echo '<div class="integrantes">';
                    foreach ($integrantes as $integrante) {
                        echo '<div class="integrante">' . $integrante->nome . '</div>';
                    }
                    echo '</div>';
                }
            } else {
                $rs = $connection->prepare("SELECT nome FROM integrantes");
                $rs->execute();
                $integrantes = $rs->fetchAll(PDO::FETCH_OBJ);
                echo '<h2>Todos os Integrantes:</h2>';
                echo '<div class="integrantes">';
